...
class SeeAllRadiologii extends Component {
    ...
    componentDidMount() {
        fetch('http://localhost:8080/radiologies/byStatus')
    ...
    render() {
        ...
        return(
            <tr key={radiologie.id}>
                ...
                <td>{this.getValueOrDash(radiologie.dimensiune)}</td>

                <td>
                    <ButtonGroup>
                        <Button size="sm" className="action-button edit-button" onClick={() => {
                            this.props.navigate(`/radiolog/${radiologie.id}`);
                            window.location.reload();
                        }}>Adauga radiologie</Button>
        ...
        return (
            <div className='main-content-container-radiolog'>
                <HeaderRadiolog/>
